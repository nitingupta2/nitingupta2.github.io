...
# Run lm models with different seeds and take the mean metrics
set.seed(1000)
...
dfModels <- expand_grid(seed = vRandomSeeds,
                        predictors = list("ALL")) %>%
    mutate(results = map2(seed, predictors, ~ cross_validate_model(.x, dfTrain, .y, "glm"))) %>%
...
model_fit <- fit_model(x_train = lTransform$x_train, y_train = lTransform$y_train %>% log(), model_method = "glm")
...
# create_submission_entry(model_fit, lTransform$x_test, vTestIDs, "submit_glm.csv")
