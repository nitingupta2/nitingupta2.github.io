...
dfModels <- expand_grid(seed = vRandomSeeds,
                        predictors = list("ALL")) %>%
    mutate(results = map2(seed, predictors, ~ validate_model(.x, dfTrain, .y, "xgbLinear", is_scaled = TRUE))) %>%
...
lTransform <- get_transformed_data(dfTrain, dfTest, is_scaled = TRUE)
model_fit <- fit_model(x_train = lTransform$x_train, y_train = lTransform$y_train %>% log(), model_method = "xgbLinear")
...
# create_submission_entry(model_fit, lTransform$x_test, vTestIDs, "submit_xgbLinear.csv")
