...
fit_rf_model <- function(randomSeed, formulaString, get_model = FALSE) {
    
    print(glue::glue("{Sys.time()} - seed: {randomSeed} - Running randomForest model: {formulaString}"))
    ...
    # fit the model
    model_rf <- train(x_train, y_train, method = "rf", trControl = trainControl(method = "cv", number = 3, allowParallel = T))
    ...
    if(get_model) {
        return(list(model_rf = model_rf, dfResults = dfResults))
    } else {
        return(dfResults)
    }
}
...
dfModels <- expand_grid(seed = vRandomSeeds, 
    ...
    mutate(results = map2(seed, model, ~ fit_rf_model(.x, .y))) %>% 
...
best_model_results <- fit_rf_model(1333, formulaString_best, get_model = T)

formulaString_base <- "price ~ ."
base_model_results <- fit_rf_model(1333, formulaString_base, get_model = T)
...
saveFilePath <- paste0(parentFolderPath, "/content/casestudies/diamonds-part3-non-linear-models/fit_models_rf.Rda")
save(dfModels, best_model_results, formulaString_best, file=saveFilePath)
